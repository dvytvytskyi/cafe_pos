...
interface FaqItem {
  id: number;
  category: 'Ordering' | 'Loyalty' | 'Payments' | 'Dietary' | 'Account & Merch';
  question: string;
  answer: string;
}
...
export default function FaqPage() {
  ...
  const categories = ['All', 'Ordering', 'Loyalty', 'Payments', 'Dietary', 'Account & Merch'];
  ...
  return (
    <div className="h-screen overflow-y-auto bg-gray-50 pb-[120px] relative scroll-smooth">
      {/* Header section (matching /menu & /loyalty header styling) */}
      ...
      <div className="max-w-[480px] mx-auto px-6 pt-5 flex flex-col gap-5">
        ...
        <div className="relative w-full">
          ...
          <input 
            ...
            className="w-full bg-white border border-gray-200/80 focus:border-black rounded-[16px] py-3 pl-10 pr-4 text-xs font-semibold text-gray-900 placeholder:text-gray-400 transition-all outline-none shadow-xs"
          />
          {searchQuery && (
            <button 
        ...
        <div className="flex flex-col gap-3">
          {filteredFaqs.length === 0 ? (
            <div className="bg-white rounded-[24px] p-8 text-center flex flex-col items-center gap-3 border border-gray-200/60">
            ...
            filteredFaqs.map((faq) => {
              ...
              return (
                <div 
                  key={faq.id}
                  className="bg-white rounded-[20px] border border-gray-200/60 shadow-xs overflow-hidden transition-all"
                >
                  <button
                    onClick={() => toggleFaq(faq.id)}
                    className="w-full p-4 flex items-start justify-between gap-3 text-left cursor-pointer hover:bg-gray-50/50 transition-colors"
                  >
                    <div className="flex flex-col gap-1 flex-1">
                      <span className="text-[9px] font-bold text-[#FDBD38] uppercase tracking-wider">
                        {faq.category}
                      </span>
                    ...
                    <div className={`w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center flex-shrink-0 transition-transform duration-200 ${isExpanded ? 'rotate-180 bg-gray-200' : ''}`}>
        ...
        {/* Contact Support Options (WhatsApp, Telegram, Email) */}
        <div className="bg-white rounded-[24px] p-5 border border-gray-200/60 shadow-xs flex flex-col gap-4 mt-2">
          <div className="flex flex-col gap-1">
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <a
              ...
              className="w-full bg-[#25D366] hover:bg-[#20bd5a] text-white font-bold text-sm py-3 px-4 rounded-full transition-all active:scale-[0.98] flex items-center justify-center gap-2 shadow-xs"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-full bg-[#229ED9] hover:bg-[#1d8ec4] text-white font-bold text-sm py-3 px-4 rounded-full transition-all active:scale-[0.98] flex items-center justify-center gap-2 shadow-xs"
            >
            ...
            <a
              ...
              className="w-full bg-gray-100 hover:bg-gray-200 text-gray-900 font-bold text-sm py-3 px-4 rounded-full transition-all active:scale-[0.98] flex items-center justify-center gap-2 border border-gray-200/60"
            >
