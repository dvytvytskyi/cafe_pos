...
export async function POST(req: Request) {
  try {
    ...
    console.log('Received verified Glovo Order Cancellation:', event);

    // Publish to BullMQ for asynchronous background execution
    await queue.publish('glovo:order_cancelled', event);

    return NextResponse.json({ success: true }, { status: 200 });
  } catch (error) {
    console.error('Glovo Webhook Cancellation Error:', error);
