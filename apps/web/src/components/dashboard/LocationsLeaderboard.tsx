            ...
            {sorted.length === 0 ? (
              ...
              sorted.map((loc, index) => {
                ...
                return (
                  <div
                    key={loc.id}
                    className={`flex flex-col p-4 rounded-2xl transition-colors cursor-pointer border flex-shrink-0 ${selectedLocation === loc.id ? 'border-[#EE635E] bg-[#EE635E]/10' : 'border-gray-100 hover:border-gray-200 hover:bg-gray-50'}`}
                    onMouseEnter={() => {
                      setSelectedLocation(loc.id);
                      setHoverSource('list');
                    }}
