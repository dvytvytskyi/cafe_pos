  ...
  useEffect(() => {
    sorted.forEach((loc) => {
      ...
      if (selectedLocation === loc.id) {
        el.style.zIndex = '50';
        dotDiv.className =
          'marker-dot w-4 h-4 rounded-full border-2 border-white shadow-sm bg-[#f59e0b] transition-all duration-300';
        if (nameDiv) {
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <div
                          className={`p-2 rounded-xl transition-colors ${selectedLocation === loc.id ? 'bg-[#f59e0b] text-white' : 'bg-gray-100 text-gray-500'}`}
                        >
