...
export default function PhotoProofUpload({ item, isOpen, onClose, onUpload }: PhotoProofUploadProps) {
  ...
  const handleFileSelected = async (file: File | null) => {
    ...
    setIsUploading(true);
    try {
      const result = await uploadPhotoAsync(file);
      onUpload(result.url);
    } catch (err) {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="p-6 space-y-6">
            ...
            {error && (
              <div role="alert" data-testid="photo-upload-error" className="bg-red-50 border border-red-100 text-red-700 text-[13px] font-medium rounded-xl px-3 py-2">
                {error}
              </div>
            )}
            ...
            {item.photoUrl ? (
              <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
                ...
                <button type="button" onClick={onClose} className="w-full btn-secondary-corgi">
                  Close
                </button>
              </div>
            ) : (
                  ...
                  {isUploading ? (
                    ...
                    <>
                      ...
                      <span className="text-[14px] font-bold text-gray-700 group-hover:text-gray-900 transition-colors">
                        Click to upload JPEG or PNG (max 5MB)
                      </span>
                    </>
                  )}
                </button>
